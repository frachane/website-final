...
function form() {
	...
	var yesSmoke = document.querySelector('#yesSmoke');
	var noSmoke = document.querySelector('#noSmoke');
	...
	var cheque = new createjs.Bitmap("images/Cheque.png");
	// stage.addChild(cheque);

	cheque.x = stageW/2;
	cheque.y = stageH/2;

	cheque.setBounds(0, 0, 345.45, 122.62);

	zim.centerReg(cheque);

		var rent = document.querySelector('#rent');


		rent.addEventListener("click", function rentCheque(){
			stage.removeAllChildren();
			stage.addChild(cheque);
		})

		rent.addEventListener("input", function price(){
			var price = new createjs.Text(rent.value, "25px Handlee", "black");

			price.x = stageW/2+80;
			price.y = stageH/2;
			stage.addChild(price);

			rent.addEventListener("keydown", function removePrice(e){
				if(e.keyCode == 8){
					stage.removeChild(price);
					// stage.removeAllChildren();
				}
			});
			...
			var spriteSheet1 = new createjs.SpriteSheet({
					framerate: 8,
					"images": ["images/Map-Etobicoke.png"],
					"frames": {"regX": 0, "height": 200, "count": 12, "regY": 0, "width": 400},
			...
			var spriteSheet2 = new createjs.SpriteSheet({
					framerate: 8,
					"images": ["images/Map-NorthYork.png"],
					"frames": {"regX": 0, "height": 200, "count": 12, "regY": 0, "width": 400},
					// define two animations, run (loops, 1.5x speed) and jump (returns to run):
					"animations": {
						"zoom": [0, 11, false],
			...
			var spriteSheet3 = new createjs.SpriteSheet({
					framerate: 8,
					"images": ["images/Map-Scarborough.png"],
					"frames": {"regX": 0, "height": 200, "count": 12, "regY": 0, "width": 400},
					// define two animations, run (loops, 1.5x speed) and jump (returns to run):
					"animations": {
						"zoom": [0, 11, false],
			...
			var spriteSheet6 = new createjs.SpriteSheet({
					framerate: 8,
					"images": ["images/Map-EastYork.png"],
					"frames": {"regX": 0, "height": 200, "count": 12, "regY": 0, "width": 400},
					// define two animations, run (loops, 1.5x speed) and jump (returns to run):
					"animations": {
						"zoom": [0, 11, false],
			...
			var spriteSheet8 = new createjs.SpriteSheet({
				...
				var spriteSheet9 = new createjs.SpriteSheet({
						framerate: 8,
						"images": ["images/Map-Downtown.png"],
						"frames": {"regX": 0, "height": 200, "count": 12, "regY": 0, "width": 400},
						// define two animations, run (loops, 1.5x speed) and jump (returns to run):
						"animations": {
							"zoom": [0, 11, false],
				...
				var spriteSheet10 = new createjs.SpriteSheet({
					...
					var mapEtobicoke = new createjs.Sprite(spriteSheet1, "zoom");
					mapEtobicoke.setBounds(0,0,400,200);
					zim.centerReg(mapEtobicoke);

					var mapNorthYork = new createjs.Sprite(spriteSheet2, "zoom");
					mapNorthYork.setBounds(0,0,400,200);
					zim.centerReg(mapNorthYork);

					var mapScarborough = new createjs.Sprite(spriteSheet3, "zoom");
					mapScarborough.setBounds(0,0,400,200);
					zim.centerReg(mapScarborough);

					var mapNorth = new createjs.Sprite(spriteSheet4, "zoom");
					mapNorth.setBounds(0,0,400,200);
					zim.centerReg(mapNorth);

					var mapYork = new createjs.Sprite(spriteSheet5, "zoom");
					mapYork.setBounds(0,0,400,200);
					zim.centerReg(mapYork);

					var mapEastYork = new createjs.Sprite(spriteSheet6, "zoom");
					mapEastYork.setBounds(0,0,400,200);
					zim.centerReg(mapEastYork);

					var mapMidTown = new createjs.Sprite(spriteSheet7, "zoom");
					mapMidTown.setBounds(0,0,400,200);
					zim.centerReg(mapMidTown);

					var mapWest = new createjs.Sprite(spriteSheet8, "zoom");
					mapWest.setBounds(0,0,400,200);
					zim.centerReg(mapWest);

					var mapDowntown = new createjs.Sprite(spriteSheet9, "zoom");
					mapDowntown.setBounds(0,0,400,200);
					zim.centerReg(mapDowntown);

					var mapEast = new createjs.Sprite(spriteSheet10, "zoom");
					mapEast.setBounds(0,0,400,200);
					zim.centerReg(mapEast);


					var region = document.querySelector('#region');

					region.addEventListener("input", function map(){
						if(region.value == "Etobicoke"){
							stage.removeAllChildren();
							mapEtobicoke.x = stageW / 2;
							mapEtobicoke.y = stageH / 2;
							stage.addChild(mapEtobicoke);
						}
						else if (region.value == "North York"){
							stage.removeAllChildren();
							mapNorthYork.x = stageW / 2;
							mapNorthYork.y = stageH / 2;
							stage.addChild(mapNorthYork);
						}

						else if (region.value == "Scarborough"){
							stage.removeAllChildren();
							mapScarborough.x = stageW / 2;
							mapScarborough.y = stageH / 2;
							stage.addChild(mapScarborough);
						}
						else if (region.value == "Toronto North"){
							stage.removeAllChildren();
							mapNorth.x = stageW / 2;
							mapNorth.y = stageH / 2;
							stage.addChild(mapNorth);
						}
						else if (region.value == "York"){
							stage.removeAllChildren();
							mapYork.x = stageW / 2;
							mapYork.y = stageH / 2;
							stage.addChild(mapYork);
						}
						else if (region.value == "East York"){
							stage.removeAllChildren();
							mapEastYork.x = stageW / 2;
							mapEastYork.y = stageH / 2;
							stage.addChild(mapEastYork);
						}
						else if (region.value == "Toronto Central"){
							stage.removeAllChildren();
							mapMidTown.x = stageW / 2;
							mapMidTown.y = stageH / 2;
							stage.addChild(mapMidTown);
						}
						else if (region.value == "Toronto West"){
							stage.removeAllChildren();
							mapWest.x = stageW / 2;
							mapWest.y = stageH / 2;
							stage.addChild(mapWest);
						}
						else if (region.value == "Downtown Toronto"){
							stage.removeAllChildren();
							mapDowntown.x = stageW / 2;
							mapDowntown.y = stageH / 2;
							stage.addChild(mapDowntown);
						}
						else if (region.value == "Toronto East"){
							stage.removeAllChildren();
							mapEast.x = stageW / 2;
							mapEast.y = stageH / 2;
							stage.addChild(mapEast);
						}
					});

			var phoneDial = new createjs.Bitmap("images/Phone.png");

			phoneDial.x = stageW/2;
			phoneDial.y = stageH/2+10;

			phoneDial.setBounds(0, 0, 159, 182);

			zim.centerReg(phoneDial);

				var phone = document.querySelector('#phone');


				phone.addEventListener("click", function dialPad(){
					stage.removeAllChildren();
					stage.addChild(phoneDial);
				});

				var phoneButton = new zim.Circle(18,"lightgrey");

				function dialAppear(){
					createjs.Tween.get(phoneButton)
					.to({alpha:0.8}, 200, createjs.Ease.bounceIn)
					.to({alpha:0}, 200, createjs.Ease.bounceOut);
				}

				phone.addEventListener("keydown", function press(e){
					if(e.keyCode == 49 || e.keyCode == 97){
						console.log("1");
						stage.addChild(phoneButton);
						phoneButton.x = stageW/2-47;
						phoneButton.y = stageH/2-54;
						dialAppear();
					}
					else if (e.keyCode == 50 || e.keyCode == 98){
						console.log("2");
						stage.addChild(phoneButton);
						phoneButton.x = stageW/2;
						phoneButton.y = stageH/2-54;
						dialAppear();
					}
